...
- **Editor assistant** (extra): a short editorial brief, suggested actions, and simple Q&A ("which channel performed best?", "what should we publish next?") over the same filtered data. Rule-based, not a live LLM, but useful in a demo without faking an API call.
